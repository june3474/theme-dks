...
#+begin_src css
  ...
  menubutton {
      -gtk-icon-source: -gtk-scaled(url("objects/other/open-menu-symbolic.svg"));
      margin: 0px 10px;
  }
...
#+begin_src css
  ...
  /* gtk3에서 사용했던 header와 stack의 접촉면 border 숨기기는 잘 안됨.
     stack이 먼저 그려지고 header가 나중에 그려지는 header.bottom이나 header.right는
     gtk3 방식이 작도하나 반대인 top과 left는 안됨.
     stack에 header 위치 클래스가 붙어 있으면 좋을텐데 아쉽게도 없음.
     일단 임시책으로 두 부류를 나눠서 처리함 */
  ...
  notebook > header.top + stack:not(:only-child) {
      border-top: none;
  }

  notebook > header.bottom + stack:not(:only-child) {
      border-bottom: none;
  }

  notebook > header.right + stack:not(:only-child) {
      border-right: none;
  }

  notebook > header.left + stack:not(:only-child) {
      border-left: none;
  }
  ...
  /*remove border of button--esp, close button--'s border */
