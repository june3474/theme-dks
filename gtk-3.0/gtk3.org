...
#+begin_src css
  ...
  /* titlebutton - minimize, maxmize, close buttons */
  ...
  windowcontrols button.titlebutton.restore {
      background-image: -gtk-scaled(url("objects/title-buttons/titlebutton-restore.svg"), url("objects/title-buttons/titlebutton-restore@2.svg"));
      background-color: transparent;
  }
  ...
  windowcontrols button.titlebutton.restore:backdrop {
      background-image: -gtk-scaled(url("objects/title-buttons/titlebutton-restore-backdrop.svg"), url("objects/title-buttons/titlebutton-restore-backdrop@2.svg"));
      background-color: transparent;
  }
  ...
  windowcontrols button.titlebutton.restore:backdrop:hover {
      background-image: -gtk-scaled(url("objects/title-buttons/titlebutton-restore-hover.svg"), url("objects/title-buttons/titlebutton-restore-hover@2.svg"));
      background-color: transparent;
  }
  ...
  windowcontrols button.titlebutton.restore:active {
      background-image: -gtk-scaled(url("objects/title-buttons/titlebutton-restore-active.svg"), url("objects/title-buttons/titlebutton-restore-active@2.svg"));
      background-color: transparent;
  }
...
#+begin_src css
  ...
  treeview:backdrop row:selected {
      color: @fg_color;
      background-color: @bg_color;
  }
  ...
  treeview:disabled row:selected {
      color: @insensitive_fg_color;
  ...
  treeview > dndtarget.after:drop(active) {
      border-top-style: none;
  }

  treeview > dndtarget.before:drop(active) {
...
#+begin_src css
   ...
   window {
       background-color: @bg_color;
       /* decoration node가 없을 경우 fallback */
       border: 1px solid @border_color;
       /* border-radius: 6px 6px 0px 0px; */
       /* 아래에 적은 것처럼 xfce에서는 무시될 가능성이 높지만 해가 될 건 없음.
          다른 wm을 사용할 경우도 있으니...
          Anyhow, shadow는 theme 설정과 무관하게 compositor로 그려질 수 있다. */
   ...
   /*
      decoration 노드는 Window Manager나 client-side-decoration(csd)가
      Application에서 활성화(enabled) 여부에 따라 있을 수도 있고 없을 수도 있음.
      decoration 노드가 있는 경우 window 노드의 property를 오버라이드함.
      다만, window > decoration은 윈도우의 일부(타이틀바나 외곽선)에만 적용됨.
      전체 윈도우에 색을 입히고 싶으면 window 스타일이 적용됨.
      참고로, gtk4는 decoration node가 없고 .csd, .ssd 등이 style class로 사용됨.

      XFCE의 Xfwm4는 기본적으로 server-side-decoration(ssd)을 사용하기 때문에
      decoration 노드가 없을 수도 있으나 csd가 활성화 되어 있으면 나타남.
      XFCE(Xfwm4)는 csd를 부분적으로 지원하며, 많은 경우 제대로 된 그림자, 그림테두리,
      리사이즈 핸들 등은 제대로 작동하지 않을 수 있음.

      ✅ 적용되지 않거나 제한적인 CSS 속성들 -- from ChatGPT 
      속성      설명        window / decoration에서의 동작
      padding   내부 여백   무시되거나 시각적으로 영향 없음 (내용에만 영향)
      margin    외부 여백       무시됨. 윈도우는 "부모"가 없기 때문에 쓸모 없음
      box-shadow    그림자      많은 경우 무시됨, 특히 Xfwm4/X11 환경
      border-radius 테두리     CSD 환경에서는 부분적으로 작동, X11에서는 거의 무시됨
      opacity   투명도      GTK 자체는 지원하지만, WM이 지원 안 하면 무시됨
      transform 회전,스케일    적용되지 않음. layout 레벨에서만 작동
      background-image      일부 환경에서는 동작 안 함, 특히 decoration 노드
      min-width/min-height  최소 크기   window 노드에는 무시됨
      display   none 등     GTK CSS는 display 속성을 아예 지원하지 않음

      ✅ 반대로 적용 잘 되는 속성들
      속성               설명
      border            테두리 (CSD에서 유용)
      background-color  배경색
      color             일부 텍스트 색상 (headerbar 등 자식에게 영향)
      border-image      (제한적이지만) 작동
      outline           테두리 대체 가능 (border와 별도)
      visibility        hidden은 되지만 완전히는 숨기지 못할 수 있음
  ,*/
